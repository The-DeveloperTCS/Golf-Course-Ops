...
import { getSpecificTerminal } from "redux/terminal/service";
import NotificationActions from "redux/notifications/actions";
import useRolePermissions from "hooks/usePermissionAsPerAssign";

const TerminalForm = (props) => {
  const { terminalId, updateTerminal } = props;
  const [updatedTerminal, setUpdateTerminal] = useState({ ...updateTerminal });
  ...
  const onSave = () => {
    ...
    props.onSave({ ...updatedTerminal }).then(() => setSaving(false));
  };

  const title = () => {
    if (updatedTerminal.id) {
      return `Update Terminal #${updatedTerminal.id}`;
    }
                ...
                <div className="form-group row">
                  ...
                  <div className="col-sm-8">
                    <div className="pretty p-default p-curve p-toggle">
                      <input
                        type="checkbox"
                        checked={updatedTerminal.status}
                        disabled={!useTerminalPermission}
                        onChange={(e) => {
                          setUpdateTerminal({
                            ...updatedTerminal,
