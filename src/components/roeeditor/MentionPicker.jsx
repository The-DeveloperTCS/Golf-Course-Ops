  ...
  const filteredUsers = useMemo(() => {
    if (users && users.length) {
      ...
      return isQuery !== ""
        ? users.filter(
            (user) =>
              user.username.toLowerCase().includes(isQuery.toLowerCase()) ||
              user.first.toLowerCase().includes(isQuery.toLowerCase()) ||
              user.last.toLowerCase().includes(isQuery.toLowerCase())
          )
        : users;
    }
    return null;
      ...
      {open && (
        <div ref={mentionMenu} className="mention-picker">
          {filteredUsers && filteredUsers.length ? (
              ...
              {filteredUsers.map((user, i) => {
                return (
                  <div
                    ...
                    onClick={() => insertMentionHandler(user.username)}
                  >
                    <div>
                    ...
                    <div>
                      ...
                      <div className="fs-12 medium-text">{user.username}</div>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="fs-16 medium-text">No users found..</div>
          )}
        </div>
      )}
    </Fragment>
