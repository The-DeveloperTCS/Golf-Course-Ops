...
const ImageUploader = ({ onUploadStart, onUploadReady, onError }) => {
  ...
  async function uploadImage(pendingImage) {
    onUploadStart();
    pendingImage.forEach(uploadItem);
  }

  const uploadItem = async (row, index) => {
    if (!["image/png", "image/jpeg"].includes(row.type)) {
      onError("Invalid image format. Allowed image formats are PNG and JPEG");
      return;
    }
    fetchDigitalOceanUrlForImage().then((res) => {
      try {
        handleSubmission(res.data.url, row.type, row);
      } catch (err) {
        console.log(err);
      }
    });
  ...
  const handleSubmission = (url, contentType, selectedFile) => {
    ...
    reader.onload = async (event) => {
      ...
      fetch(url, { method: "PUT", body: resizedImage, headers })
        .then(async (result) => {
