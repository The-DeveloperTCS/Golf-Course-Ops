import React, { useMemo } from "react";
import { connect, useDispatch } from "react-redux";
import { useTable, useSortBy, useFilters, usePagination } from "react-table";
import classnames from "classnames";
import ReactTableWrapper from "./reacttbl.style";
import { history } from "redux/store";
import { updateBanner } from "redux/onboardingBanner/service";
import NotificationActions from "redux/notifications/actions";
import BannerActions from "redux/onboardingBanner/action";

const { bannersList } = BannerActions;
const { successWithTimeout } = NotificationActions;

const HeaderComponent = (props) => {
  let classes = {
    "my-2": true,
    "mx-3": true,
    "-sort-asc": props.isSortedDesc !== undefined && !props.isSortedDesc,
    "-sort-desc": props.isSortedDesc !== undefined && props.isSortedDesc,
  };
  return <div className={classnames(classes)}>{props.title}</div>;
};

const DataTable = (props) => {
  const { data, bannersList } = props;
  const dispatch = useDispatch();

  const sortBy = useMemo(() => {
    return props.sortBy || [];
  }, [props.sortBy]);
  const columns = useMemo(
    () =>
      props.columns.map((c) => {
        const colDef = {
          Header: (tableInstance) => {
            return (
              <HeaderComponent
                isSortedDesc={tableInstance.column.isSortedDesc}
                title={c.title}
              />
            );
          },
          placeholder: c.title,
          accessor: c.field,
          disableFilters: !c.enableFilters,
        };

        if (c.id) {
          colDef.id = c.id;
        }

        if (c.cell) {
          colDef.Cell = c.cell;
        }
        if (c.enableFilters) {
          colDef.Filter = FilterComponent;
        }

        return colDef;
      }),
    [props.columns]
  );

  const {
    getTableProps,
    getTableBodyProps,
    prepareRow,
    page,
    headerGroups,
  } = useTable(
    {
      data: props.data,
      columns: columns,
      initialState: {
        pageSize: 25,
        pageIndex: 0,
        sortBy: sortBy,
      },
    },
    useFilters,
    useSortBy,
    usePagination
  );

  const onEdit = (bId, e) => {
    e.preventDefault();
    e.stopPropagation();
    history.push("/onboarding-banner/" + bId);
  };

  const onDelete = (bId, e) => {
    e.preventDefault();
    e.stopPropagation();
    const banners = data.filter((elem) => elem.id !== bId);

    return updateBanner({ banners })
      .then((res) => {
        dispatch(
          NotificationActions.successWithTimeout(
            `Banner #${bId} Delete successfully!`
          )
        );
        bannersList();
      })
      .catch((err) => {
        dispatch(
          NotificationActions.failure(
            `Banner #${bId} failed to update! ${err.response.data.message}`
          )
        );
      });
  };

  return (
    <ReactTableWrapper {...props}>
      <div className="roe-card-style mt-15 mb-30">
        <div className="roe-card-header">
          <span className="hash"># </span> {props.title}
        </div>
        <div className="table-container text-center overflow-auto">
          <table
            border={1}
            className="custom-react-table-theme-class"
            {...getTableProps()}
          >
            <thead>
              {headerGroups.map((headerGroup) => (
                <tr {...headerGroup.getHeaderGroupProps()}>
                  {headerGroup.headers.map((header) => (
                    <th
                      {...header.getHeaderProps(header.getSortByToggleProps())}
                    >
                      <div>{header.render("Header")}</div>
                    </th>
                  ))}
                </tr>
              ))}
            </thead>
            <tbody {...getTableBodyProps()}>
              {headerGroups.map((headerGroup) => (
                <tr {...headerGroup.getHeaderGroupProps()}>
                  {headerGroup.headers.map((header) => {
                    return (
                      <td
                        {...header.getHeaderProps(
                          header.getSortByToggleProps()
                        )}
                      >
                        <div>
                          {header.canFilter ? header.render("Filter") : null}
                        </div>
                      </td>
                    );
                  })}
                </tr>
              ))}
              {page.map((row, index) => {
                prepareRow(row);
                return (
                  <tr key={index + 1}>
                    <th scope="row">{index + 1}</th>

                    <td>
                      {
                        <img
                          src={`${row.original.image}`}
                          alt="banner"
                          style={{ width: "250px", height: "100px" }}
                        />
                      }
                    </td>
                    <td>{row.original.url}</td>
                    <td>
                      <button
                        className="btn c-btn-sm c-outline-danger ma-5"
                        onClick={(e) => onDelete(row.original.id, e)}
                      >
                        <i className="fa fa-trash" aria-hidden="true"></i>
                      </button>
                      <button
                        className="btn c-btn-sm c-outline-primary ma-5"
                        onClick={(e) => onEdit(row.original.id, e)}
                      >
                        <i className="fas fa-edit"></i>
                      </button>
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>
      </div>
    </ReactTableWrapper>
  );
};

const FilterComponent = (tableInstance) => {
  const { filterValue, setFilter } = tableInstance.column;
  return (
    <input
      type="text"
      value={filterValue || ""}
      onChange={(e) => {
        setFilter(e.target.value || undefined); // Set undefined to remove the filter entirely
      }}
      className="tabl-search react-form-input"
      placeholder={`${tableInstance.column.placeholder}`}
      onClick={(e) => e.stopPropagation()}
    />
  );
};

export default connect(null, { bannersList })(DataTable);
