...
export default function StickyHeadTable(props) {
  ...
  const [allCouriers, setCouriers] = useState([]);
  const [batchData, setBatchData] = useState([]);
  const csvLink = useRef();
  const usePendingPaymentsPermission = useRolePermissions("PENDING_PAYMENTS");
  ...
  const getCourierName = (cId) => {
    if (cId !== null) {
      const couriers = allCouriers.filter((cr) => cr.id === cId);
        ...
        <tbody>
          ...
          {props.data.map((i) => (
            <tr key={i.id}>
              ...
              <td>{i.bankReferenceNumber}</td>
              <td>{getCourierName(i.courierId)}</td>
              <td>{i.courierInvoiceNumber}</td>
              <td>{i.total}</td>
              <td>{i.comment}</td>
              <td>
                {i.status === "PENDING" ? (
